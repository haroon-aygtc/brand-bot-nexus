...
import axios, { AxiosRequestConfig, AxiosResponse, AxiosError } from "axios"; // Import axios and its types
...
export async function apiRequest<T = any>(
  ...
  if (env.IS_DEVELOPMENT && mockResponse) {
...
export const api = {
  ...
  clearCacheFor: (method: string, url: string, params?: any, data?: any) => {
    const cacheKey = `${method}:${url}:${JSON.stringify(
      params
    )}:${JSON.stringify(data)}`;
    cache.delete(cacheKey);
  },
