...
const ApiKeyManager = () => {
  ...
  useEffect(() => {
    const loadUsageStats = async () => {
      try {
        const usageStats = await apiKeyService.getApiKeyUsageStats();
        setUsageStats(usageStats);

        setLastUsed({
          gemini: usageStats.lastUsed.gemini,
          huggingFace: usageStats.lastUsed.huggingface,
        });
      } catch (error) {
  ...
  const onSubmit = async (data: ApiKeyFormValues) => {
    ...
    try {
      if (activeTab === "gemini") {
        ...
        const success = await apiKeyService.setHuggingFaceApiKey(data.huggingFaceApiKey);
        if (!success) {
          throw new Error("Failed to save Hugging Face API key");
        }
      }
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm font-medium">
                          {usageStats.usageStats.gemini?.totalCalls.toLocaleString()}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="text-sm font-medium">
                          {usageStats.usageStats.gemini?.successRate}%
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="text-sm font-medium">
                          {usageStats.usageStats.gemini?.averageResponseTime}s
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="text-sm font-medium">
                          ${usageStats.usageStats.gemini?.costThisMonth}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="rounded-md border p-4 bg-muted/50">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm font-medium">
                          {usageStats.usageStats.huggingface?.totalCalls.toLocaleString()}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="text-sm font-medium">
                          {usageStats.usageStats.huggingface?.successRate}%
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="text-sm font-medium">
                          {usageStats.usageStats.huggingface?.averageResponseTime}s
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-xs text-muted-foreground">
                        ...
                        <p className="text-sm font-medium">
                          ${usageStats.usageStats.huggingface?.costThisMonth}
                        </p>
                      </div>
                    </div>
                    <div className="mt-3 flex items-center">
